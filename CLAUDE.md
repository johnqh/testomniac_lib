...
This is the primary hook consumed by UI layers (testomniac_app, testomniac_app_rn).
...
- **testomniac_app** — Web frontend that consumes `useHistoriesManager` from this library
- **testomniac_app_rn** — React Native app that consumes `useHistoriesManager` via file: links
- **testomniac_api** — Backend server; this library communicates with it indirectly through testomniac_client

## Coding Patterns

- `useHistoriesManager` is the primary hook -- it orchestrates testomniac_client hooks + Zustand store into a single unified interface for UI layers
